...
import service.DiagnosticAIService;
...
public class DiagnosticController {
    ...
    private final DiagnosticAIService aiService = new DiagnosticAIService();
    ...
    private VBox creerCarte(Diagnostic d) {
        ...
        date.setStyle("-fx-text-fill: #1F2937; -fx-font-weight: bold;");
        Label etat       = new Label("État: "         + d.getEtat());
        etat.setStyle("-fx-text-fill: #334155;");
        Label symptomes  = new Label("Symptômes: "    + d.getSymptomes());
        symptomes.setStyle("-fx-text-fill: #334155;");
        Label infos      = new Label("Infos: "         + d.getInformationsComplementaires());
        infos.setStyle("-fx-text-fill: #475569;");
        Label disease    = new Label("Maladie: "       + d.getDiseaseDetected());
        disease.setStyle("-fx-text-fill: #2E7D32; -fx-font-weight: bold;");
        Label confidence = new Label("Confiance IA: "  + String.format("%.2f", d.getConfidence()) + "%");
        confidence.setStyle("-fx-text-fill: #1F2937;");

        for (Label lbl : List.of(etat, symptomes, infos, disease, confidence)) {
    ...
    private void remplirFormulaire(Diagnostic d) {
        ...
        if (txtInformations != null) txtInformations.setText(d.getInformationsComplementaires());
    ...
    // ══════════════════════════════════════════════════════════════════════
    // SAUVEGARDER / MODIFIER (avec appel IA)
    // ══════════════════════════════════════════════════════════════════════
    @FXML
    private void sauvegarderDiagnostic() {
        ...
        String symptomes = txtSymptomes.getText().trim();
        String plantType = txtPlantType != null ? txtPlantType.getText().trim() : "";
        String leafColor = txtLeafColor != null ? txtLeafColor.getText().trim() : "";
        String humidity = txtHumidity != null ? txtHumidity.getText().trim() : "";
        String temperature = txtTemperature != null ? txtTemperature.getText().trim() : "";

        // Appel au modèle IA (script ou API configuré dans config.properties)
        DiagnosticAIService.DiagnosticAIResult aiResult = aiService.runDiagnostic(etat, symptomes, plantType, leafColor, humidity, temperature);

        try {
            if (selectedDiagnostic == null) {
                Diagnostic d = new Diagnostic();
                remplirDiagnostic(d, aiResult);
                service.ajouter(d);
                showAlert("Succès", "Diagnostic ajouté ! Maladie détectée : " + aiResult.getDiseaseDetected());
            } else {
                remplirDiagnostic(selectedDiagnostic, aiResult);
    ...
    // ══════════════════════════════════════════════════════════════════════
    // REMPLIR OBJET DIAGNOSTIC (avec résultat IA)
    // ══════════════════════════════════════════════════════════════════════
    private void remplirDiagnostic(Diagnostic d, DiagnosticAIService.DiagnosticAIResult aiResult) {
        d.setIdCulture(culture != null ? culture.getIdCulture() : 0);
        d.setIdUser(userId);
        d.setEtat(txtEtat.getText().trim());
        d.setSymptomes(txtSymptomes.getText().trim());
        d.setInformationsComplementaires(txtInformations != null ? txtInformations.getText().trim() : buildInfosComplementaires());
        d.setDiseaseDetected(aiResult.getDiseaseDetected());
        d.setConfidence(aiResult.getConfidence());
        d.setRapportIa(aiResult.getRapportIa());
        d.setDateDiagnostic(java.time.LocalDate.now());
    }

    private String buildInfosComplementaires() {
        StringBuilder sb = new StringBuilder();
        if (txtPlantType != null && !txtPlantType.getText().trim().isEmpty()) sb.append("Type: ").append(txtPlantType.getText().trim()).append("; ");
        if (txtLeafColor != null && !txtLeafColor.getText().trim().isEmpty()) sb.append("Couleur feuilles: ").append(txtLeafColor.getText().trim()).append("; ");
        if (txtHumidity != null && !txtHumidity.getText().trim().isEmpty()) sb.append("Humidité: ").append(txtHumidity.getText().trim()).append("%; ");
        if (txtTemperature != null && !txtTemperature.getText().trim().isEmpty()) sb.append("Température: ").append(txtTemperature.getText().trim()).append("°C.");
        return sb.toString();
    }

    // ══════════════════════════════════════════════════════════════════════
    // VALIDATION (txtInformations peut être absent dans le FXML)
    // ══════════════════════════════════════════════════════════════════════
    private boolean validerSaisie() {
        if (culture == null || culture.getIdCulture() <= 0) {
            showAlert("Validation", "Veuillez d'abord sélectionner une culture avant d'enregistrer un diagnostic !");
            return false;
        }
    ...
    private void clearForm() {
        txtEtat.clear();
        txtSymptomes.clear();
        if (txtInformations != null) txtInformations.clear();
        txtPlantType.clear();
        if (txtLeafColor   != null) txtLeafColor.clear();
        if (txtSpotSize    != null) txtSpotSize.clear();
        if (txtHumidity    != null) txtHumidity.clear();
