...
public class DashboardAdminController {
    ...
    private void handleDeleteUser(User user) {
        ...
        conf.showAndWait().filter(b -> b == ButtonType.OK)
                .ifPresent(b -> {
                    try {
                        userService.supprimer(user.getId()); // peut lancer SQLException
    ...
    private void handleToggleBlockUser(User user) {
        ...
        try {
            userService.modifier(user); // peut lancer SQLException
            showSuccess(user.getEtatCompte() == EtatCompte.BLOQUE ? "🔒 Bloqué !" : "🔓 Débloqué !");
            handleRefresh();
        } catch (SQLException e) {
            e.printStackTrace();
            showError("❌ Impossible de modifier l'utilisateur : " + e.getMessage());
        }
    }
